...
// Demo for the "read tool renders the image inline" feature. Written as a real
// on-disk pirpc/Anthropic `.jsonl` so it flows through the production
// /messages tail-read path (readSessionMessagesTail + the toSessionMessages
// fan-out that lifts toolResult image blocks onto tool.images) rather than the
// mock adapter's pre-shaped messages. The read tool's toolResult carries a real
// PNG image block; expanding the tool card should render the actual chart, not
// just the "[Read image file …]" note. Used by
// tests/playwright/read-tool-image-render.spec.ts.
const readImageId = 'seeded-read-image';
const readImageFile = path.join(root, '0000000000014_seeded-session-read-image.jsonl');
const readChartPath = path.resolve('tests/playwright/fixtures/demo-read-chart.png');
const readChartBase64 = (await fs.readFile(readChartPath)).toString('base64');
const readMdPath = path.resolve('tests/playwright/fixtures/demo-notes.md');
await fs.writeFile(readMdPath, '# Weekly report\n\nKey takeaways:\n\n- **Desktop** sessions are trending up\n- *Mobile* is catching up fast\n- See `metrics.csv` for the raw numbers\n\n> Generated by the analytics job.\n', 'utf8');
const readMd = await fs.readFile(readMdPath, 'utf8');
const isoOfRead = (ms) => new Date(ms).toISOString();
let readTs = 1700000014000;
const readLines = [];
readLines.push({ type: 'session', id: readImageId, cwd, sessionName: 'Read image render demo', timestamp: isoOfRead(readTs) });
readTs += 1000;
readLines.push({
  type: 'message', id: 'ri-u-0', timestamp: isoOfRead(readTs),
  message: { role: 'user', content: [{ type: 'text', text: 'Read preview/demo-read-chart.png and tell me what the chart shows.' }] },
});
readTs += 1000;
const readCallId = 'toolu_read_chart';
readLines.push({
  type: 'message', id: 'ri-a-0', timestamp: isoOfRead(readTs),
  message: { role: 'assistant', content: [
    { type: 'text', text: 'Let me open the screenshot.' },
    { type: 'toolCall', id: readCallId, name: 'read', arguments: { path: 'preview/demo-read-chart.png' } },
  ] },
});
readTs += 200;
readLines.push({
  type: 'message', id: 'ri-tr-0', timestamp: isoOfRead(readTs),
  message: { role: 'toolResult', toolCallId: readCallId, content: [
    { type: 'text', text: 'Read image file [image/png]\n[Image: original 910x546, displayed at 910x546.]' },
    { type: 'image', data: readChartBase64, mimeType: 'image/png' },
  ] },
});
// Second read: a markdown file, to show rich markdown rendering of a read result.
readTs += 1000;
const readMdCallId = 'toolu_read_md';
readLines.push({
  type: 'message', id: 'ri-a-1', timestamp: isoOfRead(readTs),
  message: { role: 'assistant', content: [
    { type: 'toolCall', id: readMdCallId, name: 'read', arguments: { path: 'preview/demo-notes.md' } },
  ] },
});
readTs += 100;
readLines.push({
  type: 'message', id: 'ri-tr-1', timestamp: isoOfRead(readTs),
  message: { role: 'toolResult', toolCallId: readMdCallId, content: [{ type: 'text', text: readMd }] },
});
readTs += 1000;
readLines.push({
  type: 'message', id: 'ri-a-2', timestamp: isoOfRead(readTs),
  message: { role: 'assistant', content: [{ type: 'text', text: 'The chart shows weekly active sessions: desktop trending up and mobile catching up.' }] },
});
await fs.writeFile(readImageFile, readLines.map((l) => JSON.stringify(l)).join('\n') + '\n', 'utf8');
console.log(`seeded ${readImageFile} (${readLines.length} jsonl records)`);
